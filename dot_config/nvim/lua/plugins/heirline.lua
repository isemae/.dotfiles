local config = function()
  ...
  local NeoTest = {
    ...
    },
    {
      ...
      init = function(self)
        self.adapter =
          vim.split(vim.split(self.adapter_ids[1], ":", { plain = true })[1], "neotest-", { plain = true })[2]
      end,
  ...
  local QuickFix = {
    {
      provider = function(self)
        ...
        return string.format("%s %s:(%s/%s) ", self.quickfix.nr, self.quickfix.title, idx, self.quickfix.size)
      end,
    },
  }
